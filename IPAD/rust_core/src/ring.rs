...
pub struct Ring {
    ...
    /// Totaal aantal bytes dat ooit in de ring geschreven is (stats).
    pub bytes_in: AtomicU64,
    /// Gezet door de UDP-thread wanneer er packet loss/reordering buiten het
    /// herstelvenster is opgetreden. De parser-thread leest en cleart dit elke
    /// iteratie: bij `true` wordt de opgebouwde (mogelijk corrupte) `pending`
    /// buffer weggegooid en wordt opnieuw gezocht naar een geldige startcode,
    /// in plaats van blind door te plakken over het gat.
